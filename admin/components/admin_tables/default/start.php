...
/*if (isset($_GET["ELEMENT_ID"])) {
    ...
    if (!isset($GLOBALS["NEWS_FILTER"])) $GLOBALS["NEWS_FILTER"] = array();
    ...
    if ($db_page_items && $db_items) {
        ...
        for ($i = 1; $i <= $pages_links_count; $i++) {?>
            <?
            if (stripos($url,'&PAGE=')) {
                $url_dols = explode('&PAGE=', $url);
                $url_left_subdols = explode("&",$url_dols[1]);
                if (isset($url_left_subdols[1])) $url = $url_dols[0] . "&PAGE=" . $i . "&" . $url_left_subdols[1];
                else $url = $url_dols[0] . "&PAGE=" . $i;
            } else {
                $url .= "&PAGE=". $i;
            }?>
            <a href="<?=$url?>" class="<?if($GLOBALS["PAGE"] == $i) echo "current"?>"><?=$i?></a>
        <?}
        ?>
            </div>
        <?
